...
The operating rules of `my-day-os` — the "kernel contract." ADRs record *decisions*; this document is the *living specification* those decisions ratify into. It answers four questions:
...
- **L3 — Views never mutate truth.** Ring-3 surfaces (Web, Obsidian, Notion) may read and may *submit intents*; they may never write the SoR directly.
- **L4 — Edits are messages, not shared writes.** Human input (including Notion HID edits) enters as input events in a message-passing model — never as in-place mutation of shared state. (Resolves the cache-coherency problem.)
- **L5 — No unprivileged irreversible action.** No irreversible real-world action (send email, decline invite, delete) occurs without an explicit, granted capability **and** confirmation. A hallucination cannot act in your name.
...
- **C1 — Min external poll interval / rate ceiling.** Respect device "bus bandwidth" (e.g. Notion ~3 req/s). *(value: TBD)*
...
- **Laws** — change only by a superseding ADR that explicitly names the law being altered. Never edited silently.
- **Limits** — change deliberately; record the old→new value and the reason here.
- **Levers** — tune freely in config/runtime; no ADR required. Defaults live here once ADR-0003 lands.
